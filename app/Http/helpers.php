...
if (! function_exists('generate_activation_code')) {
    function generate_activation_code(int $digit = 6): int
    {
        return rand((int)str_repeat('1', $digit), (int)str_repeat('9', $digit));
    }
}

if (! function_exists('get_next_wp_username')) {
    function get_next_wp_username(string $username): string
    {
        $number = User::where('username', 'like', $username.'%')->count() ?: 0;

        if ($number == 0) {
            return $username.'-'.random_str(6);
        }

        if ($number == 1) {
            return $username.'-1-'.random_str(6);
        }

        $number = $number;

        return $username.'-'.$number.'-'.random_str(6);
    }
}